...
Когда появляется `tun0`, установленный net-device hotplug-скрипт автоматически
добавляет маршрут по умолчанию в таблицу `vpn`.
...
поэтому следующий запуск Ansible не перезапишет параметры подключения. Роль
перезапускает sing-box, только если конфигурация проходит `sing-box check`;
сервис с начальной конфигурацией остаётся остановленным для ручной настройки.
В inventory должна быть группа `[openwrt]`.
...
Скрипт удаления убирает правила доменной маршрутизации и загруженный им список
доменов, но сохраняет прочие include-файлы dnsmasq, а также пакет и конфигурацию
sing-box.
