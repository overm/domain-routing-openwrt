...
эквивалентный `route.default_interface`/`outbound.bind_interface`. Без привязки
исходящих соединений к внешнему интерфейсу трафик может вернуться в `tun0` и
образовать петлю. Установщик не перезаписывает существующий JSON, поэтому этот
параметр нужно проверить и в собственном конфиге.
...
TUN-зона принимает ответный трафик к локальным сокетам роутера, но новые
перенаправления из неё остаются запрещены. Перед каждой загрузкой скрипт до
